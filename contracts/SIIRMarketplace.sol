...
contract SIIRMarketplace {
    ...
    function list(address company, uint256[] ids, uint128 askPrice, uint32 currencyId) public {
        tvm.accept();
    ...
    function delist(address company, uint256[] ids) public {
        tvm.accept();
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
    ...
    /// message; the marketplace holds it until acceptance/cancellation.
    /// @param company the CompanySIIR address
    ...
    function bid(address company, uint256[] ids, uint128 price, uint32 currencyId, uint64 validUntil) public internalMsg {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _bidCount++;
            _bids[_bidCount] = Bid(msg.sender, company, ids[i], price, currencyId, validUntil, false);
            emit BidPlaced(_bidCount, msg.sender, company, ids[i], price, currencyId);
        }
    }

    /// Bidder withdraws an open offer (funds return in currencyId).
    ...
    /// open and price-compatible with the ask.
    ...
    /// forwards each track's delta to the listing seller.
    function claimAndForward(address company, uint256[] ids, uint32[] currencyIds) public {
        tvm.accept();
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            uint256 listingId = _activeListing[company][id];
    ...
    /// since the claim snapshot was taken for each listed deed.
    /// @param company the CompanySIIR address
    /// @param ids     SIIR ids to settle
    /// @param currencyIds tracks to forward (SHELL=2, eccUSDC=3, ...)
    function settleClaims(address company, uint256[] ids, uint32[] currencyIds) public {
        tvm.accept();
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            uint256 listingId = _activeListing[company][id];
